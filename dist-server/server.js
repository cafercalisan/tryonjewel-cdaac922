...
var ANALYSIS_MODEL_FALLBACK = "gemini-2.5-flash";
var IMAGE_GEN_PRIMARY = "gemini-3-pro-image-preview";
var IMAGE_GEN_FALLBACK = "gemini-3.1-flash-image-preview";
...
async function generateSingleImage(base64Images, prompt, userId, imageRecordId, index, _unused, jobId, aspectRatio = "3:4", startTemperature = 0.12) {
  ...
  const OVERLOAD_BACKOFF = [5e3, 3e3, 1e4];
  const NORMAL_BACKOFF = [3e3, 2e3, 5e3];
  const temperatures = [
    startTemperature,
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      try {
        ...
        await query("UPDATE processing_jobs SET error_message = $1 WHERE id = $2", [`\xDCretim hatas\u0131: ${errMsg.substring(0, 300)}`, jobId]);
      } catch (_) {
      }
      return null;
    }
...
OUTPUT: A single professionally retouched jewelry image from the ${angle.label}, on pure white background. Ultra high resolution.`.trim();
    ...
    if (generatedUrls.length === 0) {
      const existingJob = await queryOne("SELECT error_message FROM processing_jobs WHERE id = $1", [jobId]);
      const detailedError = existingJob?.error_message || "AI g\xF6rsel \xFCretimi ba\u015Far\u0131s\u0131z oldu. L\xFCtfen farkl\u0131 bir g\xF6rsel veya sahne ile tekrar deneyin.";
      if (!isAdminUser) {
        try {
...
async function callGeminiForBrand(prompt) {
  ...
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${ANALYSIS_MODEL2}:generateContent?key=${GOOGLE_API_KEY2}`;
  const response = await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
app.all("/api/auth/signup", handler8);
app.all("/api/auth/login", handler9);
app.all("/api/auth/refresh", handler10);
app.all("/api/auth/me", handler11);
app.all("/api/generate-jewelry", handler);
app.all("/api/generate-video", handler2);
app.all("/api/generate-design", handler3);
app.all("/api/generate-model", handler4);
app.all("/api/check-video-status", handler6);
app.all("/api/analyze-brand", handler7);
app.all("/api/admin-set-credits", handler5);
app.all("/api/admin-data", handler19);
app.all("/api/scenes", handler12);
app.all("/api/images", handler13);
app.get("/api/processing-jobs/active", handler14);
app.post("/api/processing-jobs/cancel", handler14);
app.get("/api/processing-jobs/:id", handler14);
app.all("/api/processing-jobs", handler14);
app.all("/api/profile", handler15);
app.all("/api/videos", handler16);
app.all("/api/user-models", handler17);
app.all("/api/brand-profiles", handler18);
app.all("/api/upload", handler20);
app.all("/api/signed-url", handler21);
